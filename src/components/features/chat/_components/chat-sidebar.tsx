  ...
  return (
    <div className="flex h-full flex-col overflow-hidden border-r border-gray-200 bg-white">
      <div className="px-5 pb-3 pt-5">
        <p className="text-[17px] font-extrabold text-gray-900">{t("title")}</p>
      ...
      <div className="flex-1 overflow-y-auto">
        {summaries.length === 0 && (
          <p className="px-4 py-8 text-center text-[13px] font-semibold text-gray-400">{t("no_chats")}</p>
        )}
        ...
        {closed.length > 0 && (
          <>
            <div className="mx-4 my-2 flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">{t("archive")}</span>
              <div className="h-px flex-1 bg-gray-100" />
            </div>
            {closed.map((s) => (
              <ChatRow key={s.bookingId} s={s} isActive={s.bookingId === activeBookingId} />
