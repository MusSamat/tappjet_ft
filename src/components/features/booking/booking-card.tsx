  ...
  return (
    <article className="flex flex-col gap-3 rounded-2xl border border-ink-200 bg-white p-4 dark:border-ink-800 dark:bg-ink-900">
      ...
      {role === "driver" && passenger && (
        <div className="flex items-center gap-2 border-t-[0.5px] border-ink-100 pt-3 dark:border-ink-800">
          <DriverAvatar name={passenger.name ?? "?"} src={passenger.avatarUrl ?? null} size="sm" />
          <div className="flex min-w-0 flex-1 flex-col">
            <span className="text-body text-ink-900 dark:text-white">{passenger.name}</span>
          ...
          <span className="text-caption text-ink-500 dark:text-ink-400">{booking.seatsCount} {t("seats_word")}</span>
        </div>
      )}
      ...
      {role === "passenger" && driver && (
        <div className="flex items-center gap-2 border-t-[0.5px] border-ink-100 pt-3 dark:border-ink-800">
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <span className="text-body text-ink-900 dark:text-white">{driver.name}</span>
      ...
      {booking.comment && (
        <p className="rounded-xl bg-ink-50 px-3 py-2 text-body-lg text-ink-700 dark:bg-ink-800 dark:text-ink-200">
          «{booking.comment}»
        </p>
      )}
