...
export function PriceCard({ value, label, onChange }: Props) {
  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-xs ring-1 ring-ink-100 dark:bg-ink-900 dark:ring-ink-800">
      <div className="mb-2 text-[14px] font-900 text-ink-900 dark:text-white">{label}</div>
      <div className="flex items-center gap-2 rounded-xl bg-ink-50 px-4 py-2.5 dark:bg-ink-800">
        ...
        <input
          ...
          className="w-full bg-transparent text-[16px] font-900 text-ink-900 outline-none dark:text-white"
          aria-label={label}
        />
        <span className="shrink-0 text-[13px] font-900 text-ink-400">{t("som")}</span>
      </div>
    </div>
  );
}
