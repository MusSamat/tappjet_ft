...
export function RouteCard({ origin, destination, onOrigin, onDestination, iconAccent }: Props) {
  ...
  return (
    <div className="space-y-2.5 rounded-3xl bg-white p-4 shadow-card dark:bg-ink-900">
      <div className="flex items-center gap-2 rounded-2xl bg-ink-50 px-4 py-1 dark:bg-ink-800">
        <Circle className={cn("h-4 w-4 shrink-0", iconAccent)} aria-hidden="true" />
        <CityAutocomplete borderless value={origin} onChange={onOrigin} placeholder={t("from_placeholder")} className="flex-1" />
      </div>
      <div className="flex items-center gap-2 rounded-2xl bg-ink-50 px-4 py-1 dark:bg-ink-800">
        <MapPin className="h-4 w-4 shrink-0 text-accent-500" aria-hidden="true" />
        <CityAutocomplete borderless value={destination} onChange={onDestination} placeholder={t("to_placeholder")} className="flex-1" />
      </div>
    </div>
  );
}
