...
export function PassengerCancelModal({
  ...
  return (
    <Overlay onClose={onClose}>
      <h2 className="mb-1 text-[18px] font-extrabold text-ink-900">{t("cancel_booking_title")}</h2>
      <p className="mt-1 text-[13px] text-ink-500">{t("cancel_reason_hint")}</p>
      ...
      <div className="mt-5 flex gap-3">
        <button
          type="button"
          onClick={onClose}
          className="flex-1 rounded-xl border border-ink-200 py-2.5 text-[13px] font-bold text-ink-700 hover:bg-ink-50"
        >
          {t("back_btn")}
        </button>
        <button
