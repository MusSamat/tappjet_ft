...
const TIER_COLORS: Record<Tier, {
  ...
  novice:   { textColor: "text-ink-600",   bgColor: "bg-ink-100",   ringColor: "ring-ink-400",   progressBg: "bg-ink-500",   borderColor: "border-ink-200"   },
  ...
  elite:    { textColor: "text-accent-700",  bgColor: "bg-accent-50",   ringColor: "ring-accent-400",  progressBg: "bg-accent-500",  borderColor: "border-accent-200"  },
};
...
export default function LoyaltyPage() {
  ...
  return (
    <Container className="py-6 sm:py-8">
      <h1 className="mb-5 text-[22px] font-extrabold text-ink-900 sm:text-[26px]">
        {t("title")}
      </h1>
      ...
      {statusLoading ? (
        ...
        <>
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 leading-none">
                  <span className="text-[40px] font-extrabold text-ink-900">{status?.points ?? 0}</span>
                  <span className="ml-2 text-[16px] font-semibold text-ink-500">{t("points_suffix")}</span>
                </p>
              </div>
            </div>
          ...
          <div className="mb-4 rounded-2xl border border-ink-100 bg-white p-5">
            <h2 className="mb-5 text-[15px] font-extrabold text-ink-900">{t("tiers_section")}</h2>
            <div className="relative flex items-start">
              {/* Track line */}
              <div className="absolute left-[18px] right-[18px] top-[18px] h-0.5 bg-ink-200" />
              {tierIndex > 0 && (
                <div
          ...
          {/* ── How to earn ── */}
          <div className="mb-4 rounded-2xl border border-ink-100 bg-white p-5">
            <h2 className="mb-4 text-[15px] font-extrabold text-ink-900">{t("how_to_earn_section")}</h2>
              ...
              {(["driver", "passenger", "five_stars"] as const).map((key, idx) => (
                <div key={key} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[13px] font-semibold text-ink-700">{t(`earn.${key}`)}</span>
                  </div>
                  <span className="text-[14px] font-extrabold text-brand-700">{HOW_TO_EARN_POINTS[idx]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-ink-100 bg-white">
            <div className="border-b border-ink-100 px-5 py-4">
              <h2 className="text-[15px] font-extrabold text-ink-900">{t("history_section")}</h2>
            </div>
            ...
            {txQuery.isLoading ? (
              ...
              <div className="py-12 text-center">
                <p className="text-[14px] font-semibold text-ink-600">{t("empty_title")}</p>
                ...
                {transactions.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between px-5 py-3.5">
                    <div className="min-w-0 flex-1">
                      ...
                      {tx.note && (
                        <p className="truncate text-[11px] text-ink-500">{tx.note}</p>
                      )}
                    ...
                    <span className={cn(
                      ...
                      tx.points > 0 ? "text-brand-600" : "text-coral-500",
                    )}>
