    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : "border-ink-200 text-ink-700 hover:border-sky-400",
      )}
    >
      {children}
    </button>
...
export default function CreateRequestPage() {
  ...
  return (
    <div className="mx-auto max-w-[640px] px-4 py-8">
      ...
      <h1 className="mb-1 text-[24px] font-extrabold text-ink-900">{t("title")}</h1>
      <p className="mb-6 text-[13px] text-ink-500">{t("subtitle")}</p>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="rounded-2xl border border-sky-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-[15px] font-bold text-ink-900">{t("seats_label")}</div>
              <div className="mt-0.5 text-[12px] text-ink-500">{t("seats_hint")}</div>
            </div>
            <div className="flex items-center gap-4">
              <button
                type="button"
                onClick={() => patch({ seatsNeeded: Math.max(1, draft.seatsNeeded - 1) })}
                className="flex h-9 w-9 items-center justify-center rounded-full border-2 border-ink-200 text-[18px] font-bold text-ink-700 hover:bg-ink-50"
        ...
        <button
          ...
          className={cn(
            ...
            draft.flexible ? "border-sky-200 bg-sky-50" : "border-ink-200 bg-white hover:border-sky-200",
          )}
        >
          <div className="flex items-center gap-3">
            <Zap className={cn("h-5 w-5", draft.flexible ? "text-sky-600" : "text-ink-400")} />
            <div>
              <div className="text-[14px] font-bold text-ink-900">{t("flexible_label")}</div>
              <div className="text-[12px] text-ink-500">{t("flexible_hint")}</div>
            </div>
          </div>
          <div className={cn("relative h-6 w-10 flex-shrink-0 rounded-full transition-colors", draft.flexible ? "bg-sky-600" : "bg-ink-200")}>
        ...
        <div className="rounded-2xl border border-ink-100 bg-white p-5">
          ...
          <textarea
            ...
            className="w-full resize-none rounded-2xl border-2 border-ink-200 bg-ink-50 px-4 py-3 text-[13px] font-semibold text-ink-900 outline-none focus:border-sky-500 placeholder:text-ink-400"
          />
          <div className="mt-1 text-right text-[11px] text-ink-400">{draft.comment.length}/500</div>
        </div>
