  ...
  return (
    <div className="flex items-center gap-2 py-2">
      ...
      <button
        ...
        disabled={!from && !to}
        className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full border border-ink-200 bg-white text-ink-400 hover:border-sky-300 hover:text-sky-600 disabled:opacity-30 transition-colors"
...
export default function RequestsPage() {
  ...
  const empty = (
    <div className="rounded-2xl border border-ink-100 bg-white p-8 text-center">
      ...
      <p className="text-[16px] font-bold text-ink-800">{t("no_requests")}</p>
    ...
    <div className="flex flex-col gap-2">
      ...
      <div ref={sentinel} className="flex h-8 items-center justify-center">
        ...
        {!hasNextPage && requests.length > 5 && (
          <span className="text-[11px] font-semibold text-ink-400">{t("no_more")}</span>
        )}
      </div>
    </div>
  );
      ...
      <div className="hidden lg:flex lg:justify-center" style={{ height: "calc(100vh - 64px)" }}>
        ...
        >
          ...
          <div className="overflow-y-auto bg-ink-50 px-6 py-5">
            <div className="mb-4">
              <h1 className="text-[22px] font-extrabold text-ink-900">{heading}</h1>
          ...
          <div className="overflow-y-auto border-l border-ink-200 bg-white px-6 py-5">
            {selectedRequest ? (
              ...
              <div className="flex h-full items-center justify-center">
                <p className="text-[13px] font-semibold text-ink-500">{t("select_request")}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="lg:hidden">
        ...
        >
          <div className="flex items-center justify-between border-b border-ink-100 px-5 py-4">
            <h2 className="text-[17px] font-extrabold text-ink-900">{t("filters_title")}</h2>
            <button
              type="button"
          ...
          <div className="border-t border-ink-100 px-5 py-4">
            <button
              type="button"
              onClick={() => setMobileFiltersOpen(false)}
              className="h-12 w-full rounded-2xl bg-sky-600 text-[15px] font-bold text-white hover:bg-sky-700"
            >
              {t("show_btn")}
            </button>
          </div>
        ...
        >
          <div className="flex items-center justify-between border-b border-ink-100 px-5 py-4">
            <h2 className="text-[17px] font-extrabold text-ink-900">{t("request_label")}</h2>
            <button
              type="button"
